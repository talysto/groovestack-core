...
import {
  Datagrid, DeleteWithConfirmButton, FunctionField,
  List,
  NumberField,
  SearchInput,
  SelectInput,
        ...
        <FunctionField
          ...
          render={(record: any) => (
            <div onClick={e => e.stopPropagation()}>
              <div>{record.type}</div>
              <CopyToClipboard text={record.id} onCopy={() => notify('ID Copied')}>
                <Grid container direction="row" alignItems="center">
                  <small style={{ marginRight: 5 }}>{record.id.substring(0, 6)}</small><ContentCopyIcon fontSize='inherit' />
                </Grid>
              </CopyToClipboard>
            </div>
          )}
