...
CREATE TABLE public.que_jobs (
    ...
    first_run_at timestamp with time zone DEFAULT now() NOT NULL,
...
COMMENT ON TABLE public.que_jobs IS '8';
...
--
-- Name: que_jobs_ext; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.que_jobs_ext AS
 SELECT locks.id AS lock_id,
    locks.pid AS que_locker_pid,
    ((que_jobs.args -> 0) ->> 'job_class'::text) AS sub_class,
    GREATEST(que_jobs.run_at, que_jobs.expired_at, que_jobs.finished_at) AS updated_at,
        CASE
            WHEN (locks.id IS NOT NULL) THEN 'running'::text
            WHEN (que_jobs.finished_at IS NOT NULL) THEN 'completed'::text
            WHEN (que_jobs.expired_at IS NOT NULL) THEN 'failed'::text
            WHEN (que_jobs.error_count > 0) THEN 'errored'::text
            WHEN (que_jobs.run_at < now()) THEN 'queued'::text
            ELSE 'scheduled'::text
        END AS status,
    que_jobs.id,
    que_jobs.priority,
...
CREATE UNIQUE INDEX index_identities_on_user_id_and_provider ON public.identities USING btree (user_id, provider);
